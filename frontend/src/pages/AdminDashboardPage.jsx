...
export default function AdminDashboardPage() {
  ...
  useEffect(() => {
    Promise.all([api.getStats(), api.getActiveMembers(), api.getLogs()]).then(([s, m, l]) => {
      setStats(s);
      
      const mappedM = m.map(x => {
        const bd = new Date(x.birthday);
        ...
        const utilDays = Math.ceil((nextBd - new Date(today.getFullYear(), today.getMonth(), today.getDate())) / 86400000);
        return { ...x, daysUntil: utilDays };
  ...
  const quickActions = [
    { icon: UserPlus, label: "Add Member", sub: "Register manually", to: "/admin/members" },
    { icon: Upload, label: "Import CSV", sub: "Bulk add alumni", to: "/admin/members" },
    { icon: Bell, label: "Notification Settings", sub: "Configure alerts", to: "/admin/settings" },
    { icon: RefreshCw, label: "Run Scheduler", sub: "Manual trigger", to: "#", onClick: () => {
      addToast({ message: "Scheduler triggered", type: "success" });
    }}
  ];
      ...
      {loading ? (
        ...
        <div ref={bentoRef} className="grid grid-cols-1 md:grid-cols-12 gap-4 auto-rows-min">
          ...
          <div className="bento-cell col-span-12 md:col-span-6 lg:col-span-5 bg-[var(--surface-gold)] rounded-[22px] p-[28px] flex flex-col min-h-[220px] relative overflow-hidden">
            ...
            {/* Dot indicators for multiple slider */}
            {nextBirthdays.length > 1 && (
          ...
          <div className="bento-cell col-span-12 sm:col-span-6 md:col-span-6 lg:col-span-3">
            <StatCard dark label="TOTAL MEMBERS" value={stats?.totalMembers} sub="Active members" icon={Users} />
          </div>
          <div className="bento-cell col-span-12 sm:col-span-6 md:col-span-12 lg:col-span-4">
            <StatCard label="ALUMNI" value={stats?.alumni} sub="Past members" icon={GraduationCap} />
          </div>
          ...
          <div className="bento-cell col-span-12 bg-[var(--surface-white)] border border-[var(--border-subtle)] rounded-[22px] p-[24px] overflow-hidden">
            <div className="flex justify-between items-center mb-4">
              ...
              <Link to="/admin/log" className="font-sans text-[12px] font-semibold text-[var(--surface-navy)] hover:underline">View full log →</Link>
            </div>
